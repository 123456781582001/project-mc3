...
int main()
{
	...
	Port_enuSetPinDirection(PORT_u8PIN27,PORT_u8INPUT);//button data
	...
	while(1)
	{
		Uart_RcievedData(&Resive_data1);//uart data from keypad
		if(Resive_data1!='z') //
		{
			if((Resive_data1>1)&&(Resive_data1<=10))
			{
				...
				default:
					Lcd_vidGoTo(0,4);
					Lcd_vidDisplayCharacter(Resive_data1+48); // convert char to integer lcd
